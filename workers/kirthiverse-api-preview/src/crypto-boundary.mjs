...
function decodeBase64Url(value) {
  if (typeof value !== 'string' || !/^[A-Za-z0-9_-]+$/.test(value)) {
    throw new CryptoBoundaryError('ciphertext_encoding_invalid')
  }
...
function validateVersion(version) {
  if (typeof version !== 'string' || !VERSION_PATTERN.test(version)) {
    throw new CryptoBoundaryError('encryption_key_version_invalid')
  }
  return version
}

function validateContext(context) {
  if (typeof context !== 'string' || !CONTEXT_PATTERN.test(context)) {
    throw new CryptoBoundaryError('encryption_context_invalid')
  }
  return context
}
...
function validatePlaintext(value) {
  ...
  if (encoded.byteLength === 0 || encoded.byteLength > MAX_PLAINTEXT_BYTES) {
    throw new CryptoBoundaryError('plaintext_size_invalid', 400)
  }
  return encoded
}
...
function validateCryptoKey(key) {
  ...
  if (!key.usages.includes('encrypt') || !key.usages.includes('decrypt')) {
    throw new CryptoBoundaryError('encryption_key_usage_invalid')
  }
  return key
}
...
async function resolveKey(keyVersion, options) {
  ...
  let key
  try {
    key = await resolveKey({ keyVersion })
  } catch (error) {
...
export async function encryptSensitiveString(value, context, keyVersion, options = {}) {
  ...
  try {
    encrypted = await crypto.subtle.encrypt({
      name: 'AES-GCM',
      iv,
      additionalData: aadBytes(normalizedContext, normalizedVersion),
      tagLength: 128,
    }, key, plaintext)
  } catch {
    throw new CryptoBoundaryError('encryption_failed')
  }
...
export async function decryptSensitiveString(envelope, expectedContext, options = {}) {
  ...
  if (ciphertext.byteLength < 17 || ciphertext.byteLength > MAX_PLAINTEXT_BYTES + 16) {
    throw new CryptoBoundaryError('ciphertext_size_invalid')
  }
  ...
  try {
    const decrypted = await crypto.subtle.decrypt({
      name: 'AES-GCM',
      iv,
      additionalData: aadBytes(context, keyVersion),
      tagLength: 128,
    }, key, ciphertext)
    return new TextDecoder('utf-8', { fatal: true }).decode(decrypted)
  } catch {
    throw new CryptoBoundaryError('decryption_failed', 403)
  }
}

export function assertCiphertextOnlyRecord(record, encryptedFields) {
  if (!record || typeof record !== 'object' || !Array.isArray(encryptedFields) || encryptedFields.length === 0) {
    throw new CryptoBoundaryError('ciphertext_record_policy_invalid')
  }

  for (const field of encryptedFields) {
    if (typeof field !== 'string' || field.length < 1) throw new CryptoBoundaryError('ciphertext_field_policy_invalid')
    const plaintextNames = [field, `${field}Plaintext`, `${field}_plaintext`]
    for (const name of plaintextNames) {
      if (Object.prototype.hasOwnProperty.call(record, name)) {
        throw new CryptoBoundaryError('plaintext_storage_field_forbidden', 500)
      }
    }

    const envelope = record[`${field}Ciphertext`]
    if (!envelope || typeof envelope !== 'object' || envelope.algorithm !== 'A256GCM') {
      throw new CryptoBoundaryError('ciphertext_storage_field_required', 500)
    }
